...
const plans = [
  {
    ...
    badge: null,
    color: '#1E9AD8',
    features: ['Complete Accounts Management', 'FBR-Compliant Digital Invoicing', 'Cloud-Based Access (No Install)', 'Up to 10 Invoices/month', 'Free Registration & Setup', 'Business Hours Support', 'International IP Hosting (GoDaddy)', '25+ Years Expert Team'],
  },
  {
    ...
    color: '#00A266',
    highlight: true,
    features: ['Everything in Accounts', 'Full Inventory & Stock Control', 'Purchase Orders & Stock Alerts', 'Excel Bulk Import', 'Multi-Location Inventory', 'Advanced Analytics & Reports', 'Priority Support', 'Multi-Company Management'],
  },
  {
    name: 'Full Suite',
    price: 5500,
    ...
    badge: null,
    color: '#173B64',
    features: ['Everything in Accounts + Inventory', 'Manufacturing Workflow Module', 'Bill of Materials (BOM)', 'Work-in-Progress Tracking', 'Production Planning', 'Custom Integrations', 'Dedicated Account Manager', 'Phone Support & Training'],
  },
]

// Staggered bounce-in: left card from left, center from bottom (scaled big), right from right
const CARD_ENTRIES = [
...
export function PricingSection() {
  ...
  return (
    <section id="pricing" className="relative py-28 px-6 lg:px-16 overflow-hidden">
      <div className="pointer-events-none absolute right-0 top-1/2 -translate-y-1/2 w-[600px] h-[600px] rounded-full blur-[160px] opacity-[0.08]" style={{ background: 'radial-gradient(ellipse, #00A266 0%, #1E9AD8 100%)' }} />
      ...
      <div className="mx-auto max-w-7xl relative z-10">
        ...
        >
          ...
          <p className="mt-4 text-lg text-muted-foreground">No hidden fees. Free setup. Cancel anytime.</p>
        </motion.div>
        ...
        >
          <span className={`text-sm font-semibold ${!billingYearly ? 'text-foreground' : 'text-muted-foreground'}`}>Monthly</span>
          <button
          ...
          <span className={`text-sm font-semibold flex items-center gap-1.5 ${billingYearly ? 'text-foreground' : 'text-muted-foreground'}`}>
            Yearly
            <span className="rounded-full px-2 py-0.5 text-xs font-bold" style={{ background: 'rgba(0,162,102,0.15)', color: '#00A266' }}>Save 15%</span>
          </span>
        </motion.div>

        {/* Cards — bounce-in stagger from different angles */}
              ...
              >
                ...
                <div className="relative z-10">
                  ...
                  <div className="mb-6 pb-6 border-b" style={{ borderColor: `${plan.color}20` }}>
                    ...
                    {billingYearly && (
                      <p className="mt-1 text-xs" style={{ color: '#00A266' }}>Saving PKR {Math.round(plan.price * 12 * 0.15).toLocaleString()}/year</p>
                    )}
                  </div>
